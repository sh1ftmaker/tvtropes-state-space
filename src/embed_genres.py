"""Embed the curated genre taxonomy with the SAME model as the tropes.

Because the genres are embedded by gemini-embedding-2 into the same 768-dim
space as the tropes, we can score every trope's affinity to every genre by a
plain cosine dot product -- no need to re-embed the 30,984 tropes. This is the
cheap half of making genre the primary layout metric (~110 genre vectors vs.
~31k trope vectors already cached).

Usage:
  python embed_genres.py --out out/embeddings_genres.npz --sa sa.json --location global
"""
...
from genres import GENRES, genre_text  # noqa: E402
...
def main():
    ...
    names, supers, vecs = [], [], []
    for name, sup, gloss in GENRES:
        text = genre_text(name, gloss)
        delay = 2.0
        for attempt in range(8):
            try:
                ...
                supers.append(sup)
                vecs.append(emb)
                log(f"  [{len(names):3d}/{len(GENRES)}] {name}")
                break
    ...
    np.savez_compressed(out, names=np.array(names), supers=np.array(supers),
